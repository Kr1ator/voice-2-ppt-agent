...
def _construct_llm_input_prompt(
    ...
    # Describe the editing role and PowerPoint context.
    prompt_context_parts = [
        "You are an expert AI assistant that modifies PowerPoint presentations by editing their underlying XML structure.",
    ...
    # Attach the selected XML files and task-specific context.
    prompt_data_parts = [
    ...
    # Include the optional planning result when target planning is enabled.
    if edit_plan:
        try:
    ...
    # State the output contract and XML-preservation requirements.
...
def get_relevant_xml_files_heuristic(user_prompt: str, ppt_json_data: dict, all_xml_file_paths: list) -> list:
    ...
    action_slide_keywords = ["insert picture", "chart", "diagram", "table"]
